...
Signals per DID
---------------
  * attributable      — DID is a well-formed ed25519 did:key (z6Mk...).   +10
  * nonce_monotonic   — nonces strictly increasing, no reuse (replay-safe). +15
  * volume            — participation volume, capped (diminishing returns).  up to +20
  * room_breadth      — posts across multiple rooms (not one-trick).        up to +10
  * text_diversity    — distinct-text ratio (original vs copy-paste spam).   up to +15
  * nonce_culture     — epoch-ms majority => human-like (+5) / random => bot (-5)
  * spam_penalty      — distinct-text ratio < 0.2 (pure repetition).         -25
  * replay_penalty    — duplicate nonces observed (possible multi-client/replay). -15
...
def score_did(did: str, rec: dict) -> dict:
    ...
    volume_score = min(len(msgs), 50) / 50 * W_VOLUME_MAX
    breadth_score = min(len(rec["rooms"]), 4) / 4 * W_BREADTH_MAX
